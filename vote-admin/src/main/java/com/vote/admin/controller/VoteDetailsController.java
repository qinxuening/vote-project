...
public class VoteDetailsController {
    ...
    @GetMapping(value = "/candidateVoteList")
    @ResponseBody
    public CommonResult<List<VoteDetail>> candidateVoteList() {
        return CommonResult.success(iCommonCacheService.getAllCandidateVoteResult());
    }

    @ApiOperation("查看投给该候选⼈的⽤⼾列表")
    ...
    public CommonResult<CommonPage<VoteDetails>> voteForCandidateUsersList(@RequestParam() Integer candidateId,
        ...
        Page<VoteDetails> voteDetailsPage = iVoteDetailsService.voteList(candidateId, pageSize, pageNum);
    ...
    @ResponseBody
    public CommonResult<List<VoteDetail>> VoteResult() {
        return CommonResult.success(iVoteDetailsService.getAllVoteResult());
    }
}
